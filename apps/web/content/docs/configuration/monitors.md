...
> **ℹ️ Info:** If any of the matching fields (`name`, `make`, `model`, `serial`) are set, **all** of the set ones must match to be considered a match. Use `wlr-randr` to get your monitor's name, make, model, and serial.
...
> **❗ Critical — XWayland & Coordinates:** If you use XWayland applications, **never use negative coordinates** for your monitor positions. This is a known XWayland bug that causes click events to malfunction. Always arrange your monitors starting from `0,0` and extending into positive coordinates.
...
> **⚠️ Warning — Important:** Some graphics cards require setting the `WLR_DRM_NO_ATOMIC` environment variable before mango starts to successfully enable tearing.
...
> **⚠️ Warning:** Use a `DISPLAY` value other than `:1` to avoid conflicting with mangowm.
